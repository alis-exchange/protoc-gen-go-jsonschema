...
func (s *FunctionsTestSuite) TestOneofGeneratedShape() {
	...
	// Message variants should use $ref calls.
	s.Contains(cuSection, `"ContactInfo": ContactInfo_JsonSchema_WithDefs(defs)`,
		"Message variant should use $ref call")
	s.Contains(cuSection, `"MailingAddress": Address_JsonSchema_WithDefs(defs)`,
		"Message variant should use $ref call")
...
// TestMessageFieldWithOptionsUsesRef verifies that a message-type field with
// field-level json_schema options still gets a $ref (not a broken inline schema).
func (s *FunctionsTestSuite) TestMessageFieldWithOptionsUsesRef() {
	content := s.GetGeneratedContent()

	// ConstraintDemo.shipping_address has a description override.
	// It should still use the direct $ref call.
...
// TestOneofMessageVariantWithOptionsUsesRef verifies that a message-type oneof
// variant with field-level json_schema options still gets a $ref.
func (s *FunctionsTestSuite) TestOneofMessageVariantWithOptionsUsesRef() {
	content := s.GetGeneratedContent()
	cuSection := extractGoFuncSection(content, "ComprehensiveUser_JsonSchema_WithDefs")
	s.Require().NotEmpty(cuSection)

	// contact_info has a description override in the proto but must still use $ref.
	s.Contains(cuSection, `"ContactInfo": ContactInfo_JsonSchema_WithDefs(defs)`,
		"Oneof message variant with options should use $ref call")
