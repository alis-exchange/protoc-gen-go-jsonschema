...
The schema **shape** changes for every message; the generated Go API does not.
...
- **Field metadata overrides on inline copies.** A comment or `description`
  option on a message-typed field replaces the referenced message's own
  `title`/`description` in that spot (previously both were present as `$ref`
  target plus sibling).
- **`google.protobuf.Struct`, `Value`, `ListValue` are free-form**
  (`{"type": "object"}`, `{}`, `{"type": "array"}`) — matching what
  `encoding/json` actually emits for them. Their standalone
  `*_google_protobuf_Struct_*` functions are no longer generated.
- **Recursive messages gain a private `_JsonSchema_build` function.** Existing
  `_WithDefs` names and signatures are unchanged, and packages generated by
  v0.2.x and v0.3.x compose (a caller never inspects what `_WithDefs`
  returns).
