...
- **`google.protobuf.Struct`, `Value`, `ListValue` are free-form**
  ...
  removed.
- **Recursive messages gain an unexported `_JsonSchema_build(defs)` helper**
  used by the generated file itself; it is not API. Existing `_WithDefs`
  names and signatures are unchanged, and packages generated by v0.2.x and
  v0.3.x compose (a caller never inspects what `_WithDefs` returns).
- **Dependencies are forced across files.** A message that opts out with
  ...
  run now generates, in its own file; previously the referencing file's
  `_WithDefs` call was left undefined and the package did not compile.
- **Ignored message fields force nothing.** A message reachable only through
  an `ignore: true` field no longer generates.
- **Non-finite numeric options are rejected.** `inf`, `-inf` and `nan` in
  `minimum`, `maximum`, `multiple_of`, `default_number`, `enum_number` or
  `examples_number` are generation-time errors; they previously produced Go
  that did not compile.
- **Proto file names are sanitised** in Google-type function names
  (`my-d.proto` → `my_d_google_protobuf_…`); such files previously failed to
  generate.
- **proto2 groups are supported** as message fields (inlined, or `$ref` when
  on a cycle). Editions files remain unsupported: the plugin does not declare
  editions support, so protoc rejects them.
