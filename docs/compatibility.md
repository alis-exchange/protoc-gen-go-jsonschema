...
Generated schemas target Draft 2020-12 and rely on two of its behaviors:

- **`$defs` + `$ref`** for all message references, with a `$ref` root
  ("ref-as-root").
- **Sibling keywords next to `$ref`** — field descriptions, annotations, and
  constraints on message-typed fields ride the `$ref` directly. Draft-07-era
  validators ignore siblings of `$ref`; they still validate the structure
  correctly but drop that metadata.
